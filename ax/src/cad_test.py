...
def build_search_space(num_channels: int = NUM_CHANNELS):
    ...
    width_param_names = [f"channel_{i}_width" for i in range(1, num_channels + 1)]
    max_total_width = BASE_WIDTH - (num_channels - 1) * MIN_CHANNEL_SPACING
    
    width_constraint = SumConstraint(
        parameters=[p for p in parameters if p.name in width_param_names],
        is_upper_bound=True,
        bound=max_total_width,
    )
...
def extract_channel_params(params: dict, num_channels: int = NUM_CHANNELS) -> list[dict]:
    """
...
def run_cbo_for_cad():
    ...
    print(f"Optimizing for flow rate uniformity (minimize CV)")
    ...
    for i, ch in enumerate(channels, 1):
        print(f"  Channel {i}: L={ch['length']:.2f}, W={ch['width']:.2f}, H={ch['height']:.2f} mm")
    ...
    resistances = [estimate_hydraulic_resistance(ch['length'], ch['width'], ch['height']) 
                   for ch in channels]
