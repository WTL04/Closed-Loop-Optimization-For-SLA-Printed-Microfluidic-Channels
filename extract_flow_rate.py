...
def extract_flow_rate() -> float:
    ...
    if not POST_DIR.exists():
        print("ERROR: postProcessing/flowRatePatch(name=outlet) not found - simulation likely diverged")
        return -1.0
    ...
    try:
        ...
        if len(last_line_data) >= 2:
            ...
            if flow_rate <= 0:
                print(f"WARNING: Non-positive flow rate {flow_rate}, marking as failure")
                return -1.0
            return flow_rate
        else:
