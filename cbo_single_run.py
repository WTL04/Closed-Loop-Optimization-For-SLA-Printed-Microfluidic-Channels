...
def main():
    ...
    batch_summary = df_multi.groupby("batch_id")["flow_rate_per_min"].agg(
        ["mean", "std"]
    )
    ...
    df_batches["fit_adjustment_pct"] = df_batches["fit_adjustment_pct"].replace(fit_map)
    ...
    pbounds = {
        ...
        "z_rotation_deg": (0, 90),
        "fit_adjustment_pct": (0, 6),    # categorical index
    }
    ...
    best_params, _, _ = model.compute_ax_opt(context_snapshot, verbose=False)
