...
class ContextualBayesOpt:
    ...
    def _make_row(self, c_t, x):
        ...
        # layer_thickness_um must be binary {0,1}
        if "layer_thickness_um" in row:
    ...
    def _compute_lcb(self, x, c_t=None, lam=None):
        """
        Lower Confidence Bound (LCB) for MINIMIZATION problems (like CV).
        LCB = mu - lam * sigma
        """
        if c_t is None:
            if self.c_t is None:
    ...
    def objective_lcb(self, **x):        
        """
        We MINIMIZE CV, so we MINIMIZE LCB.
        bayes_opt MAXIMIZES, so return -LCB.
        """
        lcb = self._compute_lcb(x)
        return -lcb


    def compute_bayes_opt(self, c_t, init_points=5, n_iter=1, verbose=False):
        """
        Persistent Bayesian Optimization step.
        Each call adds ONE iteration to the same optimizer.
        """
        self.c_t = c_t

        if self.optimizer is None:
            self.optimizer = BayesianOptimization(
                f=self.objective_lcb,
        ...
        # ONE optimization step per iteration
        self.optimizer.maximize(init_points=0, n_iter=n_iter)

        best_result = self.optimizer.max
        best_params = best_result["params"]
        best_lcb = -best_result["target"]

        if verbose:
            print("Best params so far:", best_params)
            print("Best LCB so far:", best_lcb)

        return best_params, best_lcb, self.optimizer
